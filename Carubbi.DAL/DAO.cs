...
namespace Carubbi.DAL
{
    ...
    public class DAO<T> : IDAO<T>
    {
        ...
        ///     <para>
        ///         Caso nenhuma propriedade tenha sido marcada com o atributo e a classe possuir um atributo chamado Id, o mesmo é
        ///         definido como chave
        ///     </para>
        /// </summary>
        ...
        protected Dictionary<string, object> ObterCamposChave(T entity)
        {
            ...
            var propriedadesChave = entityType.GetProperties()
                .Where(p => p.GetCustomAttributes(typeof(CampoChaveAttribute), true).Any());
        ...
        ///     Instancia de uma classe que terá suas propriedades refletidas como parâmetros da procedure (exceto propriedades
        ///     marcadas com CampoNaoPersistivelAttribute)
        /// </param>
        ...
        /// <param name="handler">Método de conversão de linha da tabela em entidade</param>
        /// <param name="parameters">
        ///     Instancia de uma classe que terá suas propriedades refletidas como parâmetros da procedure
        ///     (exceto propriedades marcadas com CampoNaoPersistivelAttribute)
        /// </param>
        /// <returns>Coleção de objetos da entidade corrente</returns>
        ...
        /// <summary>
        ///     Transforma uma instancia de uma classe em um dicionario de parâmetros com nome da propriedade e valor encontrado na
        ///     instancia
        /// </summary>
        ...
        private static Dictionary<string, object> ObjectToDictionary(object parameters)
        {
            ...
            switch (parameters)
            {
                ...
                default:
                    ...
                    var properties = parametersType.GetProperties().Where(p =>
                        !p.GetCustomAttributes(typeof(CampoNaoPersistivelAttribute), true).Any());
        ...
        ///     <para>
        ///         O Nome da procedure sera composto pelo prefixo de procedures definido nas convenções caso tenha um, o nome da
        ///         entidade com as regras de convenção aplicadas, e o sufixo passado como parâmetro em procedureIdentifier
        ///     </para>
        /// </summary>
        ...
        {
            return ExecuteNonQueryFullEntity(string.Format("{2}.{1}{0}_{3}", entityNameConvention(typeof(T).Name),
                DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName, procedureIdentifier));
        }
        ...
        protected int ExecuteProcedure(string procedureIdentifier, object parameters)
        {
            return ExecuteNonQuery(
                string.Format("{2}.{1}{0}_{3}", entityNameConvention(typeof(T).Name),
                    DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName, procedureIdentifier),
                parameters);
        }
        ...
        ///     <para>
        ///         Regra para o padrão de nome LISTAR: [Schema].[Prefixo][Nome da Entidade com regras de convenção de
        ///         nome]_LISTAR_[Sufixo]
        ///     </para>
        /// </summary>
        /// <param name="procedureSufix">
        ///     Sufixo do nome da procedure
        ///     <example>PorId -> SP_Cliente_LISTAR_PorId</example>
        /// </param>
        /// <param name="parameters">
        ///     Instancia de uma classe que terá suas propriedades refletidas como parâmetros da procedure
        ///     (exceto propriedades marcadas com CampoNaoPersistivelAttribute)
        ...
        {
            return ExecuteReader(
                string.Format("{3}.{2}{0}_LISTAR_{1}", entityNameConvention(typeof(T).Name), procedureSufix,
                    DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName), parameters);
        }
        ...
        private bool _escopoLocal;

        /// <summary>
        ///     Constrói a DAO utilizando o conversor padrão (por reflexão) de linha da tabela para entidade e Gerenciador de
        ///     transações (UnitOfWork) <seealso cref="UnitOfWork" />
        /// </summary>
        ...
        /// <inheritdoc />
        /// <summary>
        ///     Getter e Setter do objeto UnitOfWork, quando não definido o escopo da transação corrente acaba logo após a execução
        ///     do comando efetuado por esta classe
        /// </summary>
        ...
        /// <summary>
        ///     Constrói uma DAO informando um método de conversão da linha da tabela para entidade customizado e um gerenciador de
        ...
        public DAO(Func<IDataReader, T> convertRowHandler, UnitOfWork unitOfWork)
        {
            ...
            entityNameConvention = DataBaseConventions.EntityNameConventionHandler != null
                ? DataBaseConventions.EntityNameConventionHandler
                : t => t;
        ...
        ///     <para>
        ///         A procedure a ser chamada segue o seguinte padrão de nome: [Schema].[Prefixo][Nome da Entidade com regras de
        ///         convenção de nome]_LISTAR_TODOS
        ///     </para>
        /// </summary>
        /// <returns>Toda a tabela em forma de coleção de objetos</returns>
        public IEnumerable<T> Listar()
        {
            return ExecuteReader(string.Format("{2}.{1}{0}_LISTAR_TODOS", entityNameConvention(typeof(T).Name),
                DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName));
        }
        ...
        ///     <para>
        ///         A procedure a ser chamada segue o seguinte padrão de nome: [Schema].[Prefixo][Nome da Entidade com regras de
        ///         convenção de nome]_LISTAR_[Nome da classe de filtro com regras de convenção de nome]
        ///     </para>
        /// </summary>
        ...
        {
            return ExecuteReader(
                string.Format("{3}.{2}{0}_LISTAR_{1}", entityNameConvention(typeof(T).Name),
                    entityNameConvention(typeof(TFilter).Name), DataBaseConventions.StoredProcedurePrefix,
                    DataBaseConventions.SchemaName), filtro);
        }

        /// <summary>
        ///     Método para recuperar um objeto do banco de dados através de procedure a partir das propriedades definidas como
        ///     chave da classe
        ///     <para>
        ///         A procedure a ser chamada segue o seguinte padrão de nome: [Schema].[Prefixo][Nome da Entidade com regras de
        ///         convenção de nome]_OBTER
        ///     </para>
        /// </summary>
        ...
        public virtual T Obter(T entity)
        {
            T obj;
            obj = ExecuteReader(
                string.Format("{2}.{1}{0}_OBTER", entityNameConvention(typeof(T).Name),
                    DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName),
                ObterCamposChave(entity)).FirstOrDefault();
            return obj;
        }
        ...
        public T Salvar(T entity)
        {
            ...
            ExecuteReader(
                string.Format("{2}.{1}{0}_SALVAR", entityNameConvention(typeof(T).Name),
                    DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName),
                dr =>
                {
        ...
        public virtual int Excluir(T entity)
        {
            ...
            {
                return ExecuteNonQuery(
                    string.Format("{2}.{1}{0}_EXCLUIR", entityNameConvention(typeof(T).Name),
                        DataBaseConventions.StoredProcedurePrefix, DataBaseConventions.SchemaName),
                    ObterCamposChave(entity));
            }
            catch (DbException)
            {
                throw new ApplicationException(
                    $"Não foi possível excluir este(a) {entityNameConvention(typeof(T).Name).ToLower()} pois existem referências vinculadas à este registro");
            }
        }
